...
# Apply harmonic phonon dispersion at given T to a supercell structure using the unit cell phonon modes
# IMPORTANT: In the supercell cell the equivalent atoms should be listed continued
...
def disp_vect_supercell(number_atoms, number_q, number_mu, eigenvalues_list, eigenvectors_list, mass_list, temperature, supercell_size):
    """
    Generates a displacement vector for a given ensemble of phonons and at a given temperature for a supercell
    ...
    Inputs:
        ...
        temperature -> temperature (in K)
        supercell_size -> number of unit cells in the supercell structure
    """

    vector = []
    for _ in range(number_atoms * supercell_size):
        vector.append([0, 0, 0])
    for q_val in range(number_q):
        for mu_val in range(number_mu):
            ...
            for atom in range(number_atoms):
                ...
                for _ in range(supercell_size):
                    initial_phase = random_uniform(0, 2*np.pi)

                    vector[it_atom][0] = vector[it_atom][0] + (eigenvectors[atom][0] * amplitude * np.cos(initial_phase))
                    vector[it_atom][1] = vector[it_atom][1] + (eigenvectors[atom][1] * amplitude * np.cos(initial_phase))
                    vector[it_atom][2] = vector[it_atom][2] + (eigenvectors[atom][2] * amplitude * np.cos(initial_phase))

                    it_atom = it_atom + 1
                
    for atom in range(number_atoms * supercell_size):
...
# Define a supercell variable that indicates the amount of atoms of each type
supercell_size = 2 * 2 * 2 

# Generate a number of Monte Carlo distorted structures and save them in the desired path
path_to_save = 'distorted_structures_supercell/'
number_of_structures = 100

for num_struc in range(number_of_structures):
    result = disp_vect_supercell(num_atoms, nq, num_atoms*3, values, vectors, masses, 600, supercell_size)
    path_new = path_to_save + 'POSCAR-' + str(num_struc + 1).zfill(4)
    new_POSCAR('SPOSCAR.vasp', result, path_new)